...
class BookingCVAdapter(val action: IBookingCVAction) :
    ...
    override fun onBindHolder(item: Booking, binding: ItemListBookingBinding, adapterPosition: Int) {
        binding.apply {
            ...
            tvStatus.compoundDrawablesRelative[0].colorFilter = PorterDuffColorFilter(ContextCompat.getColor(binding.root.context, item.colorStatus), PorterDuff.Mode.SRC_IN)
            tvName.text = cv.name
